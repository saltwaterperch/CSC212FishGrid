...
public class World {
	...
	public boolean canSwim(WorldObject whoIsAsking, int x, int y) {
		...
		for (WorldObject it : inSpot) {
			...
			// if the fish is our player, it can step on things, otherwise, no
			if (isPlayer = true) {
				return true;
			} else {
				return false;
	...
	public static void objectsFollow(WorldObject target, List<? extends WorldObject> followers) {
		...
		// recentPositions is a List that contains all the coordinates (x,y) of our
		// target,
		// Aka it keeps track of where our target has been in the grid

		// What is followers?
		// Followers are the objects that we want to take on the coordinates in
		// Recentpositions
		// Aka they are the fish that we want to follow the player

		// What is target?
		// Target is the object whose positions/coordinates we are adding to the list

		// Why is past = putWhere[i+1]? Why not putWhere[i]?
		// We need putWhere[i+1] because we want to followers to start in the
		// most recent *past* position. putWhere[i] is the target's current position
		...
		for (int i = 0; i < followers.size() && i + 1 < putWhere.size(); i++) {
			...
			System.out.println(putWhere.get(i));
		}
	}
}
